/**
 * @brief This library provides a basic implementation of the most common layers
 * used in Convolutional Neural Networks (CNNs), such as convolutional layers,
 * dense layers and pooling layers. For each type of layer, the library provides
 * a common interface to perform the main operations used in CNN, such as:
 * - Feeding
 * - Forwarding
 * - Back-propagation
 */
 ...
 * @param input: pointer to the input of the layer. This is used during the
 * feed-forward stage
 * @param kernels: pointer to the kernels of the layer. The layer supports
 * multiple 3D kernels.
 * @param biases: pointer to the biases of the layer. The layer supports
 * multiple 3D biases,
 * @param output: pointer to the output of the layer. This is computed after the
 * feed-forward stage
 * @param output_activated: pointer to the activated output of the layer. This
 * is computed as the result of element-wise operation applied to the output of
 * the layer
 * @param d_input: pointer to the derivative of the input w.r.t. the output.
 * This is computed after the back-proagation of the layer
 ...
 * @param activation_type: the type of activation used by the layer to compute
 ...
 * pooling layer. For each slice of the output matrix, a 3D matrix with depth =
 * 2 (slice 0 to save row indexes, slice 1 to save column indexes) is used to
 ...
 * using a squared kernel of size size*size)
 ...
 * @param input: pointer to the input of the layer. This is used during the
 * feed-forward stage. The input matrix has height = 1, depth = 1, and width
 * equals to the n. of inputs. It's like a linear 1D array, but for
 * compatibility with other layers it's implemented as a 3D matrix.
 * @param weights: pointer to the weights of the layer. Weights are stored
 * inside a 2D matrix, where the width is equal to the height of the output, and
 * the height is equal to the width of the input
 * @param biases: pointer to the biases of the layer. Biases are stored inside a
 * 2D matrix, where the width is equal to the height of the output, and the
 * height is equal to 1
 * @param output: pointer to the output of the layer. This is computed after the
 * feed-forward stage The output matrix has height = 1, depth = 1, and width
 * equals to the n. of outputs. It's a 1D array, but for compatibility with
 * other layers it's implemented as a 3D matrix.
 * @param output_activated: pointer to the activated output of the layer. This
 * is computed as the result of element-wise activation applied to the output of
 * the layer. The output_activated matrix has height = 1, depth = 1, and width
 * equals to the n. of outputs. It's a 1D array, but for compatibility with
 * other layers it's implemented as a 3D matrix.
 * @param d_input: pointer to the derivative of the input w.r.t. the output.
 * This is computed after the back-proagation of the layer. The d_input matrix
 * has height = 1, depth = 1, and width equals to the n. of inputs. It's a 1D
 * array, but for compatibility with other layers it's implemented as a 3D
 * matrix.
 * @param activation_type: the type of activation used by the layer to compute
 * the activated output.
 * @param loaded: flag used to keep track of the origin of the pointers.
...
/**
 * @brief Initialize a convolutional layer. Each member of the layer is
...
/**
 * @brief Feed a convolutional layer, and copy the content of a 3D matrix inside
 * the "input" member of the layer
 * @param layer: pointer to the target layer
 ...
 * @param layer: pointer to the target layer
 * @param input: derivative of the input w.r.t. the output coming from the next
 * layer
...
/**
 * @brief Feed a pooling layer, and copy the content of a 3D matrix inside the
 * "input" member of the layer
 * @param layer: pointer to the target layer
 * @param input: pointer to the input to copy from
 */
...
/**
 * @brief Initialize a dense layer. Each member of the layer is dinamically
 * allocated.
 * @param layer: pointer to the target layer
 * @param input_n: length of the input
 * @param output_n: length of the output
 * @param activation_type: the type of activation function used to compute the
 * output
 */
...
/**
 * @brief Feed a dense layer, and copy the content of a 3D matrix inside the
 * "input" member of the layer
 * @param layer: pointer to the target layer
 * @param input: pointer to the input to copy from
 */
...
/**
 * @brief Initialize a softmax layer. Each member of the layer is dinamically
 * allocated.
 * @param layer: pointer to the target layer
 * @param input_n: length of the input
 */
...
/**
 * @brief Feed a softmax layer, and copy the content of a 3D matrix inside the
 * "input" member of the layer
 * @param layer: pointer to the target layer
 * @param input: pointer to the input to copy from
 */
