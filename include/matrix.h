/**
 * @brief library provides a basic implementation of a 2D/3D matrix, along with
 * the most common manipulation/computation functions used in Convolutional
 * Neural Networks (CNNs). Every function (except for initialization functions)
 * has been implemented such that every operation is performed in place, without
 * the need to use auxiliary dinamically allocated structures. This can save
 * space and perform a cleaner prediction of the memory footprint.
 */
 ...
 * In this case, the caller has the duty to free the pointed data (if
 * dinamically allocated, otherwise in statically allocated do nothing), while
 * false means that the data has been dinamically allocated by the
 * matrix2d_init(matrix2d_t *m, int height, int width) In this case, the caller
 * needs to call matrix3d_destroy(matrix3d_t* m) in order to free the matrix
 */
typedef struct {
  int height;
  int width;
 ...
 * dinamically allocated, otherwise in statically allocated do nothing), while
 * false means that the data has been dinamically allocated by the
 * matrix3d_init(matrix3d_t *m, int height, int width, int depth). In this case,
 * the caller needs to call matrix3d_destroy(matrix3d_t* m) in order to free the
 * matrix
 */
typedef struct {
  int height;
...
/**
 * @brief Initialize a 2D matrix. The data of the matrix is dinamically
 * allocated and its length will be equal to (height * width), and the "loaded"
...
/**
 * @brief Destroy a 2D matrix. The data will be deallocated only if "loaded" is
 * set to false, otherwise does nothing
 * @param m: the target matrix
 */
void matrix2d_destroy(const matrix2d_t *m);

/**
 * @brief Initialize a 2D matrix. The data of the matrix is set through the
 * base_address, and the "loaded" flag will be set to true
 * @param m: the target matrix
 * @param height: n. of rows of the matrix
 * @param width: n. of columns of the matrix
...
/**
 * @brief Perform a 180° rotation of a 2D matrix, and stores the result inside
 * the the same matrix.
 * @param input: the target matrix
 */
...
/**
 * @brief Performs an element-wise product between two 2D matrices, and stores
 * the result inside the 1st matrix.
 ...
 * allocated and its length will be equal to (height * width * depth)
 * @param m: the target matrix
 * @param height: n. of rows of the matrix
 * @param width: n. of columns of the matrix
 * @param depth: n. of channels of the matrix
...
/**
 * @brief Initialize the 3D matrix. The data of the matrix is set through the
 * base_address
 * @param m: the target matrix
 * @param height: n. of rows of the matrix
 * @param width: n. of columns of the matrix
...
/**
 * @brief Destroy a 3D matrix. The "values" member will be deallocated only if
 * "loaded" is set to false, otherwise does nothing.
 * @param m: the target matrix
 */
 ...
 * e.g. if the matrix is 2x2x2, and the data is {1.f, 2.f,
 * 3,f, 4.f, 5.f, 6.f, 7.f, 8.f}, the function will print: Layer 0 | 1.f | 2.f |
 * | 3.f | 4.f |
 * Layer 1
 * | 5.f | 6.f |
 * | 7.f | 8.f |
...
/**
 * @brief Reshape a 3D input matrix using the width/height/depth of a 3D output
 * matrix, and stores the values result into the output matrix
