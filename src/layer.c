        ...
        for (int w = 0; w < layer->output->cols_n; w++) {
          ...
          for (int i = 0; i < layer->kernel_size; i++) {
            for (int j = 0; j < layer->kernel_size; j++) {
              ...
              if (input_h < layer->input->rows_n &&
                  input_w < layer->input->cols_n) {
...
void conv_layer_backpropagation(conv_layer_t *layer,
  ...
  for (int i = 0; i < layer->kernels_n; i++) {
    matrix3d_init(&d_kernel[i], layer->kernels[i].rows_n,
                  layer->kernels[i].cols_n, layer->kernels[i].depth);
  }
